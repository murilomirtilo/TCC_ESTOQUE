<?php
    ...
    if(!empty($_GET['search'])){
      $search = $_GET['search'];
      $tabela = $pdo->prepare("SELECT id_baixaPR, avaria_produtoR, data_PR, nome_PR, patrimonio,  responsavel
      FROM baixa_pr WHERE id_baixaPR = '$search' OR patrimonio LIKE '%$search%' OR nome_PR LIKE '%$search%' OR avaria_produtoR LIKE '%$search%' OR responsavel LIKE '%$search%' OR data_PR LIKE '%$search%';");
      $tabela->execute();
      $rowTabela = $tabela->fetchAll();
  }
  else{
      $tabela = $pdo->prepare("SELECT id_baixaPR, avaria_produtoR, data_PR, nome_PR,  patrimonio, responsavel
      FROM baixa_pr ORDER BY data_PR DESC");
      $tabela->execute();
      $rowTabela = $tabela->fetchAll();
  }
...
<html lang="pt-br">
    ...
    <body>
        ...
        <br>
        <a href="exportacaoData/tabelasbaixaspr.php" style="margin-left: 5rem" class="btn btn-success btn-export" onclick="exibirPopup('Sucesso!', 'Tabela Exportada!', 'success', 'OK')">EXPORTAR TABELA</a>
    ...
    foreach ($rowTabela as $linha){
        ...
        $data_empr = strtotime($linha['data_PR']);
...
<script>
            ...
            function searchData()
            {
                window.location = 'baixasPR.php?search='+search.value; //criou um parametro
            }
        </script>
