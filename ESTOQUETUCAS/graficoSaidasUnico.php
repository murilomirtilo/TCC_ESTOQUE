...
try {
    ...
    $sql = "SELECT produto_empr, quantidade_empr, data_empr, responsavel FROM emprestimos";
    $stmt = $conn->query($sql);
    ...
    if ($stmt->rowCount() > 0) {
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $quantidadesProdutos[] = $row['quantidade_empr'];
...
<body>
<a href="tabelasEMPR.php" style="margin-left: 5rem" class="btn btn-success btn-export">VOLTAR</a>
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (context) {
                                ...
                                label += produto + ' - ' + quantidade + ' retirada(s) - ' + labelsData[dataIndex];
                                return label;
                            }
                        }
                    }
