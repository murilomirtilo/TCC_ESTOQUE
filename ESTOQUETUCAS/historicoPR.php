<?php
    ...
    $tabela = $pdo->prepare("SELECT id_emprPR, mutuario_empr, quantidade_empr, data_empr, produto_empr, patrimonio, responsavel
    FROM empr_pr;");
    $tabela->execute();
    ...
    if(!empty($_GET['search'])){
      $search = $_GET['search'];
      $tabela = $pdo->prepare("SELECT id_emprPR, mutuario_empr, quantidade_empr, data_empr, produto_empr, patrimonio, responsavel
      FROM empr_pr WHERE id_emprPR = '$search' OR patrimonio LIKE '%$search%' OR mutuario_empr LIKE '%$search%' OR produto_empr LIKE '%$search%' OR responsavel LIKE '%$search%' OR data_empr LIKE '%$search%';");
      $tabela->execute();
      $rowTabela = $tabela->fetchAll();
  }
  else{
      $tabela = $pdo->prepare("SELECT id_emprPR, mutuario_empr, quantidade_empr, data_empr, produto_empr, patrimonio, responsavel
      FROM empr_pr ORDER BY data_empr DESC;");
      $tabela->execute();
      $rowTabela = $tabela->fetchAll();
  }
...
<html lang="pt-br">
    ...
    <body>
        ...
        <div class="container">
            <b><h1 style="text-align:center;">Histórico de empréstimos</h1></b>
            <!--Barra de pesquisa-->
        <br>
        ...
        <br>
        <a href="exportacaoData/tabelasHistoricoPR.php" style="margin-left: 5rem" class="btn btn-success btn-export" onclick="exibirPopup('Sucesso!', 'Tabela Exportada!', 'success', 'OK')">EXPORTAR TABELA</a>
        ...
        <tr>
            ...
            <th scope="col"><b>MUTUÁRIO</b></th>
            <th scope="col"><b>PRODUTO EMPRESTADO</th></b>
            <th scope="col"><b>RESPONSÁVEL</th></b>
            <th scope="col"><b>DATA DO EMPRÉSTIMO</b></th>
            </tr>
        </thead>
        <tbody>
        <?php
    foreach ($rowTabela as $linha){
        echo '<tr>';
        echo '<td><b>'.$linha['patrimonio']."</b></td>";
    ...
    ?>
        ...
        <script>
            ...
            function searchData()
            {
                window.location = 'historicoPR.php?search='+search.value; //criou um parametro
            }
        </script>
...
<script>
    function redirecionarParaPaginaPR() {
        window.location.href = "tabelasPR.php";
              ...
              <li class="nav-item-baba">
                  <a onclick="redirecionarParaHistoricoPR()" style="opacity:50%;" hrfe="tabelasPR.php" class="nav-link tabelaHistoricoPR" href="#"><i class="far fa-clock"></i> Histórico </a>
              </li>
<script>
