<?php
    ...
    if(!empty($_GET['search'])){
      $search = $_GET['search'];
      $tabela = $pdo->prepare("SELECT id_entrada, quantidade_entrada, nome_entrada, data_entrada, unidade_entrada, user_entrada, quantidadeAnterior_entrada
      FROM entrada WHERE id_entrada = '$search' OR id_entrada LIKE '%$search%' OR quantidade_entrada LIKE '%$search%' OR nome_entrada LIKE '%$search%' OR data_entrada LIKE '%$search%'  OR unidade_entrada LIKE '%$search%' OR user_entrada LIKE '%$search%' OR quantidadeAnterior_entrada LIKE '%$search%';");
      $tabela->execute();
      $rowTabela = $tabela->fetchAll();
  }
  else{
      $tabela = $pdo->prepare("SELECT id_entrada, quantidade_entrada, nome_entrada, data_entrada, unidade_entrada, user_entrada, quantidadeAnterior_entrada
      FROM entrada ORDER BY id_entrada DESC;");
      $tabela->execute();
      $rowTabela = $tabela->fetchAll();
  }
...
<html lang="pt-br">
    <head>
        ...
        <title>Tabela de entradas</title>
    ...
    <body>
        <div class="container">
            <h1 style="text-align:center;">Tabela de entradas</h1>
            <!--Barra de pesquisa-->
        <br>
        ...
        <a href="exportacaoData/tabelasEntradasPUU.php" style="margin-left: 5rem" class="btn btn-success btn-export" onclick="exibirPopup()">EXPORTAR TABELA</a>
        <a href="graficoEntradas.php" style="margin-left: 5rem" class="btn btn-success btn-export">VISUALIZAR EM GRÁFICO</a>
        ...
        <tr>
            ...
            <th scope="col"><b>QUANTIADADE ANTIGA</b></th>
            <th scope="col"><b>QUANTIDADE ENTRADA</b></th>
    ...
    foreach ($rowTabela as $linha){
        echo '<tr>';
        echo "<td>".$linha['nome_entrada']."</td>";
        echo "<td>".$linha['quantidadeAnterior_entrada']."</td>";
        echo "<td>".$linha['quantidade_entrada']."</td>";
        echo "<td>".$linha['unidade_entrada']."</td>";
        echo "<td>".$linha['user_entrada']."</td>";
        $data_empr = strtotime($linha['data_entrada']);
...
<div class="modal fade" id="entradaModalLabel" tabindex="-1" role="dialog" aria-labelledby="entradaModalLabel" aria-hidden="true">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
        <form action="CRUDE/adicionar_prod.php" method="POST">
