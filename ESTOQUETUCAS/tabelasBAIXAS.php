<?php
    ...
    $tabela = $pdo->prepare("SELECT id_baixa, causa_baixa, data_baixa, nome_baixa, responsavel
    FROM baixas;");
    $tabela->execute();
  ...
  if(!empty($_GET['search'])){
    $search = $_GET['search'];
    $tabela = $pdo->prepare("SELECT id_baixa, causa_baixa, data_baixa, nome_baixa, responsavel
    FROM baixas WHERE id_baixa = '$search' OR causa_baixa LIKE '%$search%' OR data_baixa LIKE '%$search%' OR nome_baixa LIKE '%$search%' OR responsavel LIKE '%$search%';");
    $tabela->execute();
    $rowTabela = $tabela->fetchAll();
}
else{
    $tabela = $pdo->prepare("SELECT id_baixa, causa_baixa, data_baixa, nome_baixa, responsavel
    FROM baixas ORDER BY data_baixa DESC;");
    $tabela->execute();
    $rowTabela = $tabela->fetchAll();
}
...
<html lang="pt-br">
    <head>
        ...
        <title>HISTÓRICO DE BAIXAS</title>
    ...
    <body>
        ...
        <div class="container">
            <h1 style="text-align:center;">HISTÓRICO DE BAIXAS</h1>
            <!--Barra de pesquisa-->
        <br>
        ...
        <br>
        <a href="exportacaoData/tabelasbaixasPUU.php" style="margin-left: 5rem" class="btn btn-success btn-export" onclick="exibirPopup()">EXPORTAR TABELA</a>
    ...
    foreach ($rowTabela as $linha){
        echo '<tr>';
        echo "<th scope='row'><b>".$linha['id_baixa']."</th></b>";
        echo "<td>".$linha['nome_baixa']."</td>";
        echo "<td>".$linha['causa_baixa']."</td>";
        echo "<td>".$linha['responsavel']."</td>";
        $data_empr = strtotime($linha['data_baixa']);
    ...
    ?>
        ...
        <script>
            ...
            function searchData()
            {
                window.location = 'tabelasBAIXAS.php?search='+search.value; //criou um parametro
            }
        </script>
<!-- SIDEBAR -->
<div class="barraTop">
...
<li class="nav-item-baba">
                  <a onclick="redirecionarParaPaginaBaixas()" hrfe="tabelasBAIXAS.php"style="opacity:50%;" class="nav-link tabelaBaixas" href="#"><i class="fas fa-arrow-circle-down"></i> Baixas </a>
              </li>
<script>
